...
# prep shap data
shap.prep <- function(shap  = shap_result, X_train = mydata$train_mm, start=1,end){
  require(ggforce)
  # descending order
  ...
  shap_score_sub <- shap_score_sub[, names(shap$mean_shap_score)[start:end], with = F]
  shap_score_long <- melt.data.table(shap_score_sub, measure.vars = colnames(shap_score_sub))
  
  # feature values: the values in the original dataset
  fv_sub <- as.data.table(X_train)[, names(shap$mean_shap_score)[start:end], with = F]
...
plot.shap.summary <- function(data_long){
  ...
  plot1 <- ggplot(data = data_long)+
    ...
    scale_color_gradient(low='#00C5FF', high='#FF5630', 
